...
Complete the `processFrame()` method below for a two-zone processor. It should compare the average saturation of a `LEFT_ZONE` and a `RIGHT_ZONE` and assign `detectedLocation` to `SpikeLocation.LEFT` if the left zone's saturation is higher, or `SpikeLocation.RIGHT` otherwise. The zone definitions and the `SpikeLocation` enum are already provided.
