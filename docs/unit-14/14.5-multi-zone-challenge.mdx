...
sidebar_label: "14.5 · Challenge: Multi-Zone Vision"
sidebar_position: 5
---
...
# Lesson 14.5: Challenge: Multi-Zone Vision Processing for Autonomous Scoring Selection
...
The challenge combines every technique from this unit: a `VisionPortal` to open the camera stream, a custom `VisionProcessor` to analyze the frame, `Rect` zone boundaries to isolate the three candidate locations, and a result that the OpMode reads to select the correct autonomous path.
...
Using the `volatile` keyword on the field is a best practice that ensures the OpMode thread always reads the most recently written value rather than a stale cached copy.
...
public class SpikeSelectionAuto extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        // Stop streaming -- the decision is made, no more camera needed
...
3. The `__________` keyword on a shared result field ensures the OpMode thread always reads the most recently written value from the processor thread.
...
**Robot Scenario:** Complete the `processFrame()` method below for a two-zone processor. It should compare the average saturation of a `LEFT_ZONE` and a `RIGHT_ZONE` and assign `detectedLocation` to `SpikeLocation.LEFT` if the left zone's saturation is higher, or `SpikeLocation.RIGHT` otherwise. The zone definitions and the `SpikeLocation` enum are already provided.
...
public class TwoZoneProcessor implements VisionProcessor {

    public enum SpikeLocation { LEFT, RIGHT }

    public volatile SpikeLocation detectedLocation = SpikeLocation.LEFT;

    private static final Rect LEFT_ZONE  = new Rect(60,  350, 80, 80);
    private static final Rect RIGHT_ZONE = new Rect(500, 350, 80, 80);

    private final Mat hsvMat = new Mat();

    @Override
    public Object processFrame(Mat frame, long captureTimeNanos) {
        // INSERT CODE HERE: Convert frame to HSV

        // INSERT CODE HERE: Compute average saturation for LEFT_ZONE and RIGHT_ZONE
        // using Core.mean(hsvMat.submat(ZONE)).val[1]

        // INSERT CODE HERE: Set detectedLocation based on which saturation is higher
    ...
    @Override
    public void onDrawFrame(Canvas canvas, int w, int h,
                            float scaleBmp, float scaleDensity, Object ctx) {}
}
```
...
public Object processFrame(Mat frame, long captureTimeNanos) {
    ...
    double satLeft  = Core.mean(hsvMat.submat(LEFT_ZONE)).val[1];
    double satRight = Core.mean(hsvMat.submat(RIGHT_ZONE)).val[1];
