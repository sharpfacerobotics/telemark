...
public class PoseNavigationDemo extends LinearOpMode {
    ...
    public void runOpMode() {
        // Hardware setup omitted for brevity -- assume drivetrain is mapped
...
public class TagNavigator {
    ...
    public TagNavigator(AprilTagProcessor processor) {
        this.aprilTagProcessor = processor;
    }

    // INSERT CODE HERE: Write getTagDistance(int targetId).
    // Return detection.ftcPose.y if the tag is found with valid metadata.
    // Return -1.0 if the tag is not visible.
...
```java
public double getTagDistance(int targetId) {
    List<AprilTagDetection> detections = aprilTagProcessor.getDetections();
