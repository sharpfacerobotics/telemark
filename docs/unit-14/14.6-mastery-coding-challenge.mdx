...
The simulator keeps `CompetitionTeleOp` selected as the runnable entry point and carries forward the same KG-SFR DECODE robot. Your existing drivetrain, intake, transfer, launcher, and sensor code remains untouched while the camera layer is added.
