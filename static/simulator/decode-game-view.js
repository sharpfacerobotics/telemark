...
(function (global) {
  ...
  function installStyles() {
    ...
    style.textContent = ""
      ...
      + ".decode-velocity{display:flex;gap:10px;margin-top:6px;color:#aab9c6;font:500 .64rem/1.3 var(--font-code)}.decode-velocity b{color:#fde68a}"
  ...
  function createHud(container, initialMode) {
    ...
    hud.innerHTML = ""
      ...
      + "<span class=\"decode-stat\">Field supply<b data-decode-stat=\"supply\">8</b></span>"
      ...
      + "<span>measured <b data-decode-stat=\"measured\">0</b> ticks/s</span></div>"
      + "<details class=\"decode-controls\"><summary>Driver controls</summary><p>Left stick: field-relative drive · Right stick X: turn · Y: precision · X: reset heading · RB/LB: intake and transfer · Right trigger: flywheel velocity · A: launch</p></details>";
  ...
  function createProceduralField(THREE, visual, manifest) {
    ...
    const opening = manifest.goalOpening;
  ...
  function mount(options) {
    ...
    const artifactMaterial = new THREE.MeshPhongMaterial({color: 0xfbbf24, emissive: 0x713f12, emissiveIntensity: 0.18, shininess: 60});
    ...
    function artifactMesh(id) {
      if (artifacts.has(id)) return artifacts.get(id);
      const mesh = new THREE.Mesh(new THREE.CylinderGeometry(0.11, 0.11, 0.055, 20), artifactMaterial.clone());
      mesh.name = "decode-artifact-" + id;
      mesh.rotation.x = Math.PI / 2;
      mesh.castShadow = true;
      artifactGroup.add(mesh);
      artifacts.set(id, mesh);
      return mesh;
    }
    ...
    function render(snapshot) {
      ...
      snapshot.artifacts.forEach(function (artifact) {
        const mesh = artifactMesh(artifact.id);
    ...
    return Object.freeze({
      ...
      start: function () { render(game.start()); },
